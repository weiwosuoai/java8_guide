...
废话不多说，接下来，让我们进入并发 API 中最重要的执行器器： `Executor`.
...
- 2.`scheduleWithFixedDelay()` ：**固定延迟执行任务，就是说，必须要等到上个任务执行完成后才开始计时，**

**达到设定的延迟时间后，下个任务才会被触发**；
...
如上面的示例代码中这个调度任务，设置了 1s 的延时，初始化延时为 0，假设理想情况下任务的耗时就是 2s, 则任务的调度周期为： `0s -> 3s -> 6s -> 9s ....`, 它更适用于当你无法预测任务的执行时长时使用。
